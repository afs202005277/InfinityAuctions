...
@elseif($notification->type== "Wishlist Targeted")
    <div>
        ...
        <p> The auction <strong>{{ $notification->auction()->value('name') }}</strong> is matches with something on your wishlist. Go take a look!</p>
    </div>
...
@elseif($notification->type== "Auction Canceled")
    ...
    <h5 hidden>{{$notification->id }}</h5>
        ...
        @php($images = App\Models\Auction::find($notification->auction()->value('id'))->images()->get())
        <img src="{{ asset($images[0]->path) }}">
        <p> The auction <strong>{{ $notification->auction()->value('name') }}</strong> you own was cancelled! You infrieged one of our policies.</p>
    </div>
@elseif($notification->type== "Outbid")
    ...
    <h5 hidden>{{ $notification->id }}</h5>
        ...
        @php($images = App\Models\Auction::find($notification->auction()->value('id'))->images()->get())
        <img src="{{ asset($images[0]->path) }}">
        <p> You're outbid on the auction <strong>{{ $notification->auction()->value('name') }}</strong>. If still wnat the item you need to bid again.</p>
    </div>
